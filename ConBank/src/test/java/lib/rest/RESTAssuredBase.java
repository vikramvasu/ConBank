...
public class RESTAssuredBase extends PreAndTest{
	...
	public static RequestSpecification setLogs() {
		...
		RequestLogSpecification log = RestAssured
				...
				.auth()
				.preemptive()
				.basic("vairamuthu.raman@tagitmobile.com", "Thiruramu@357")
				//.basic("jabrulla.khan@tagitmobile.com", "Jaffar3110^^")
				.log();
	...
	public static void verifyResponseStatusCode(Response response, int code) {
	...
	/*public static void verifyResponseText(Response response, int text) {
		if(response.ResponseText() == text) {
			reportRequest("The response text "+text+" matches the expected text", "PASS");
		}else {
			reportRequest("The response text "+text+" does not match the expected text"+response.ResponseText(), "FAIL");	
		}
	}*/
